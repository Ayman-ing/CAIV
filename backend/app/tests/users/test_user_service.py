...
class TestUserService:
    ...
    @pytest.fixture
    async def created_user(self, db_session, sample_user_create):
    ...
    async def test_get_user_by_id_success(self, user_service, created_user):
    ...
    async def test_get_user_by_id_not_found(self, user_service):
        ...
        user = await user_service.get_user(99999)
        
        assert user is None

    async def test_get_user_by_uuid_success(self, user_service, created_user):
        """Test getting user by UUID"""
        retrieved_user = await user_service.get_user_by_uuid(str(created_user.uuid))
        
        assert retrieved_user is not None
        assert retrieved_user.uuid == created_user.uuid
        assert retrieved_user.email == created_user.email

    async def test_get_user_by_uuid_not_found(self, user_service):
        """Test getting non-existent user by UUID"""
        fake_uuid = str(uuid4())
        user = await user_service.get_user_by_uuid(fake_uuid)
        
        assert user is None

    async def test_get_user_by_email_success(self, user_service, created_user):
    ...
    async def test_list_users_with_data(self, user_service, created_user):
    ...
    async def test_list_users_pagination(self, user_service, db_session):
        ...
        # Test pagination
        users_page1 = await user_service.list_users(skip=0, limit=2)
        users_page2 = await user_service.list_users(skip=2, limit=2)
        
        assert len(users_page1) == 2
        assert len(users_page2) == 2
        assert users_page1[0].id != users_page2[0].id

    async def test_update_user_success(self, user_service, created_user):
    ...
    async def test_update_user_by_uuid_success(self, user_service, created_user):
        ...
        updated_user = await user_service.update_user_by_uuid(str(created_user.uuid), update_data)
    ...
    async def test_update_user_by_uuid_not_found(self, user_service):
        """Test updating non-existent user by UUID"""
        fake_uuid = str(uuid4())
        update_data = UserUpdate(first_name="Updated")
        
        updated_user = await user_service.update_user_by_uuid(fake_uuid, update_data)
        
        assert updated_user is None

    async def test_delete_user_by_uuid_success(self, user_service, created_user):
        """Test successful user deletion by UUID"""        
        result = await user_service.delete_user_by_uuid(str(created_user.uuid))
        
        assert result is True
        
        # Verify user is deleted
        deleted_user = await user_service.get_user_by_uuid(str(created_user.uuid))
        assert deleted_user is None

    async def test_delete_user_by_uuid_not_found(self, user_service):
        """Test deleting non-existent user by UUID"""
        fake_uuid = str(uuid4())
        
        result = await user_service.delete_user_by_uuid(fake_uuid)
        
        assert result is False

    async def test_partial_update(self, user_service, created_user):
    ...
    async def test_user_service_consistency(self, user_service, created_user):
        """Test that different access methods return consistent data"""        
        # Get user by different methods
        user_by_id = await user_service.get_user(created_user.id)
        user_by_uuid = await user_service.get_user_by_uuid(str(created_user.uuid))
