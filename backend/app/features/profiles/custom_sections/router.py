...
async def create_custom_section(
    ...
    try:
        return await service.create_custom_section(profile_uuid, section_data)
    except ValueError as e:
        raise HTTPException(status_code=400, message=str(e))

@router.get("/", response_model=List[CustomSectionResponse])
async def get_profile_custom_sections(
    profile_uuid: str,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    ...
    await check_profile_ownership(db, current_user, profile_uuid)
    service = CustomSectionService(db)
    return await service.get_sections_by_profile(profile_uuid)
...
async def update_custom_section(
    ...
    await check_profile_ownership(db, current_user, profile_uuid)
    service = CustomSectionService(db)
    section = await service.update_custom_section_by_uuid(section_uuid, section_data)
    if not section:
...
async def delete_custom_section(
    ...
    await check_profile_ownership(db, current_user, profile_uuid)
    service = CustomSectionService(db)
    if not await service.delete_custom_section_by_uuid(section_uuid):
        raise HTTPException(status_code=404, message="Custom section not found")
